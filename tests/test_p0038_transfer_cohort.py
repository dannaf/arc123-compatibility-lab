...
COHORT_PATH = REPOSITORY_ROOT / "research" / "cohorts" / "ARC12_FILENAME_HOLDOUT_007.json"


class P0038TransferCohortTests(unittest.TestCase):
    def test_p0038_is_disjoint_all_training_filename_only_cohort(self) -> None:
        cohort = json.loads(COHORT_PATH.read_text(encoding="utf-8"))
        transfer = cohort["frozen_filename_only_training_50_p0038"]
        import_paths = tuple(
        ...
        self.assertEqual(len(imports), 16)
        ...
        self.assertTrue(
            any(
                item["path"].endswith("ARC12_DEVELOPMENT_COHORT_009.json")
                for item in imports
            )
        )
        self.assertTrue(
