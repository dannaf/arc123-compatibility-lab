...
PACKET_PATH = (
    REPOSITORY_ROOT / "research" / "packets" / "P0026_ARC12_HIDDEN_ZERO_TRANSFER_50.json"
)
REPORT_ROOT = REPOSITORY_ROOT / "reports" / "P0026_arc12_hidden_zero_transfer_50"
EXPECTED_SOLVES = {
    ("arc1", "e345f17b"),
    ("arc1", "62c24649"),
    ("arc2", "ed98d772"),
...
class P0026HiddenZeroTransferTests(unittest.TestCase):
    ...
    def test_p0026_is_complete_reproducible_and_retains_limited_transfer(self) -> None:
        ...
        self.assertEqual(packet["implementation_reference"]["exact_solve_count"], 7)
        self.assertEqual(packet["implementation_reference"]["attempt_count"], 40)
        self.assertTrue(
            all(
                value is False
        ...
        self.assertIn(
            ...
            selected[("arc1", "e345f17b")],
        )
        self.assertIn("dihedral_tile", selected[("arc1", "62c24649")])
        self.assertIn("dihedral_tile", selected[("arc2", "ed98d772")])
        self.assertIn(
            "central_separator_cellwise_combine(axis=vertical",
            selected[("arc2", "0520fde7")],
        )
        self.assertFalse(
            any(
                any(
