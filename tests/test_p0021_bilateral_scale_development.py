...
PACKET_PATH = (
    ...
    / "research"
    / "packets"
    / "P0021_ARC12_BILATERAL_SCALE_DEVELOPMENT_40.json"
)
REPORT_ROOT = REPOSITORY_ROOT / "reports" / "P0021_arc12_bilateral_scale_development_40"
BASELINE_REPORT_ROOT = REPOSITORY_ROOT / "reports" / "P0020_arc12_development_baseline_40"
EXPECTED_SOLVES = {
    ("arc1", "66f2d22f"),
    ("arc1", "6430c8c4"),
    ("arc1", "0c9aba6e"),
...
NEW_SOLVES = {("arc1", "66f2d22f"), ("arc2", "b91ae062")}

sys.path.insert(0, str(REPOSITORY_ROOT / "scripts"))
import run_arc12_filename_holdout as packet_runner


class P0021BilateralScaleDevelopmentTests(unittest.TestCase):
    ...
    def test_p0021_is_complete_reproducible_and_has_two_new_exact_results(self) -> None:
        ...
        self.assertEqual(summary["exact_solve_count_by_benchmark"], {"arc1": 4, "arc2": 2})
        ...
        self.assertEqual(packet["baseline_reference"]["exact_solve_count"], 4)
        self.assertEqual(baseline["exact_solve_count"], 4)
        self.assertTrue(
            all(
                value is False
        ...
        self.assertIn(
            "adjacent_bilateral_cellwise_combine(axis=vertical",
            selected[("arc1", "66f2d22f")],
        )
        self.assertEqual(
            selected[("arc2", "b91ae062")],
            "distinct_nonbackground_scale",
        )
