...
def test_enumerator_builds_type_correct_compositions_not_named_rules():
    ...
    assert "synth[components4 -> select_unique_area -> crop_bbox]" in names
...
def test_synthesizes_unique_area_extract_across_min_and_max_worlds():
    # Demo 1: the unique-area component is the *smallest* one (area 2;
    # the other two both have area 4). Demo 2: the unique-area component is
    # the *largest* one (area 5; the other two both have area 2). A fixed
    # min/max selector therefore cannot explain both demonstrations.
    train = (
        (
            _g([
                [0, 0, 0, 0, 0, 0, 0, 0],
    ...
    assert "synth[components4 -> select_unique_area -> crop_bbox]" in names
    assert "synth[components4 -> select_unique_min_area -> crop_bbox]" not in names
...
def test_synthesis_uses_all_demonstrations_not_first_demo_only():
    # Demo 1 alone supports min-area. Demo 2 is included in the same search
    # objective and rules out max-area; there is no first-demo pruning path.
    train = (
        (
            _g([
                [0, 0, 0, 0, 0],
