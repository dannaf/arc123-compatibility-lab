...
P0042_PACKET_PATH = (
    ...
    / "research"
    / "packets"
    / "P0042_ARC12_MIXED_PARTITION_DEVELOPMENT_BASELINE_40.json"
)
P0042_REPORT_ROOT = REPOSITORY_ROOT / "reports" / "P0042_arc12_mixed_partition_development_baseline_40"
REPORT_ROOT = REPOSITORY_ROOT / "reports" / "P0043_arc12_separator_arrow_guided_panel_stamp_development_40"
EXPECTED_SOLVES = {
    ("arc2", "4c177718"),
    ("arc2", "59341089"),
    ("arc2", "cf5fd0ad"),
...
class P0043SeparatorArrowGuidedPanelStampDevelopmentTests(unittest.TestCase):
    ...
    def test_p0043_retains_complete_same_cohort_development_artifacts(self) -> None:
        packet = json.loads(PACKET_PATH.read_text(encoding="utf-8"))
        baseline_packet = json.loads(P0042_PACKET_PATH.read_text(encoding="utf-8"))
        summary = json.loads((REPORT_ROOT / "receipt.json").read_text(encoding="utf-8"))
        baseline_summary = json.loads(
            (P0042_REPORT_ROOT / "receipt.json").read_text(encoding="utf-8")
        )
        ...
        self.assertEqual(packet["cohort_import"], baseline_packet["cohort_import"])
        self.assertEqual(summary["attempt_count"], 40)
        self.assertEqual(len(tasks), 40)
        self.assertEqual(summary["exact_solve_count"], 3)
        self.assertEqual(summary["exact_solve_count_by_benchmark"], {"arc1": 0, "arc2": 3})
        ...
        self.assertEqual(baseline_summary["exact_solve_count"], 2)
        self.assertEqual(
            summary["exact_solve_count"] - baseline_summary["exact_solve_count"],
            1,
        )
        self.assertEqual(packet["development_baseline_reference"]["exact_solve_count"], 2)
        self.assertIn("same-cohort", packet["claim_boundary"])
        self.assertIn("not fresh transfer evidence", packet["claim_boundary"])
        ...
        self.assertEqual(
            selected[("arc2", "4c177718")],
            "separator_arrow_guided_panel_stamp",
        )
        self.assertIn("dihedral_tile", selected[("arc2", "59341089")])
        self.assertIn("dihedral_tile", selected[("arc2", "cf5fd0ad")])
        self.assertEqual(
            sum(
        ...
        arrow_diagram = (
            REPORT_ROOT / "arc2" / "4c177718" / "corpus_callosum.svg"
        ).read_text(encoding="utf-8")
        self.assertIn("arrow-guided panel stamp", arrow_diagram)
