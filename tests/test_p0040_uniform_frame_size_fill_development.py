...
PACKET_PATH = (
    ...
    / "research"
    / "packets"
    / "P0040_ARC12_UNIFORM_FRAME_SIZE_FILL_DEVELOPMENT_40.json"
)
P0039_PACKET_PATH = (
    REPOSITORY_ROOT
    / "research"
    / "packets"
    / "P0039_ARC12_TRAINING_DEVELOPMENT_BASELINE_40.json"
)
P0039_REPORT_ROOT = REPOSITORY_ROOT / "reports" / "P0039_arc12_training_development_baseline_40"
REPORT_ROOT = REPOSITORY_ROOT / "reports" / "P0040_arc12_uniform_frame_size_fill_development_40"
EXPECTED_SOLVES = {
    ("arc1", "74dd1130"),
    ("arc1", "dae9d2b5"),
    ("arc1", "ac0a08a4"),
...
class P0040UniformFrameSizeFillDevelopmentTests(unittest.TestCase):
    ...
    def test_p0040_retains_complete_same_cohort_development_artifacts(self) -> None:
        packet = json.loads(PACKET_PATH.read_text(encoding="utf-8"))
        p0039_packet = json.loads(P0039_PACKET_PATH.read_text(encoding="utf-8"))
        summary = json.loads((REPORT_ROOT / "receipt.json").read_text(encoding="utf-8"))
        p0039_summary = json.loads(
            (P0039_REPORT_ROOT / "receipt.json").read_text(encoding="utf-8")
        )
        ...
        self.assertEqual(summary["exact_solve_count_by_benchmark"], {"arc1": 4, "arc2": 0})
        ...
        self.assertEqual(p0039_summary["exact_solve_count"], 3)
        self.assertEqual(p0039_summary["attempt_count"], 40)
        ...
        self.assertNotEqual(
            ...
            p0039_packet["frozen_controller"]["commit"],
        )
        ...
        self.assertIn("dihedral_transform", selected[("arc1", "74dd1130")])
        self.assertIn("adjacent_bilateral_cellwise_combine", selected[("arc1", "dae9d2b5")])
        self.assertEqual(selected[("arc1", "ac0a08a4")], "distinct_nonbackground_scale")
        self.assertEqual(
            selected[("arc1", "c0f76784")],
            "uniform_frame_size_fill(fill_map=1x1:6;2x2:7;3x3:8)",
        )
        self.assertEqual(
            sum(
        ...
        frame_diagram = (
            REPORT_ROOT / "arc1" / "c0f76784" / "corpus_callosum.svg"
        ).read_text(encoding="utf-8")
        self.assertIn("frame-size interior fill", frame_diagram)
