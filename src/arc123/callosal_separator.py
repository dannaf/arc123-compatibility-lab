...
observed effect.  It exhaustively searches descriptor subsets from smallest to
largest and returns the first subset whose keys make the effect deterministic.
...
descriptor vocabulary and the configured maximum separator arity.
"""
...
def _deterministic_table(
    ...
    for observation in observations:
        try:
            ...
            return None
        prior = table.get(key)
        if prior is not None and prior != observation.effect:
            return None
...
def learn_minimal_separator(
    ...
    *,
    ...
    """Find the smallest deterministic descriptor subset.

    Search is exhaustive by arity and lexical descriptor order.  Therefore, for
    a finite candidate descriptor set D and arity cap k, if any subset of D of
    size <= k deterministically realizes the observed effect table, this
    function returns one of minimum cardinality.  Ties are lexical and hence
    reproducible.

    `require_effect_variation=True` prevents a vacuous zero-arity/constant
    explanation from being treated as a discovered semantic dependency.
    """
