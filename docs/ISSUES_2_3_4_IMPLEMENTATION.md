...
The issue's final curated acceptance condition is **not claimed complete**. `P0005-ARC12-PERSISTENT-THEORY-CURATED-60` establishes the initial persistent-theory measurement at `1/60`. `P0006-ARC12-RESIDUAL-DIHEDRAL-TILE-CURATED-60` adds one generic residual-directed family and records `4/60` exact with all 56 failures retained as complete V&V-backed reports. The three new successes follow an input-derived orientation/blank template through residual targeting and ordered composition; the row-span success retains explicit counterexamples. Independent trace review, broader transfer, and frozen generalization remain required before this blocker can be closed.
