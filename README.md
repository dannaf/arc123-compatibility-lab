...
- `P0038-ARC12-CROSS-SEPARATOR-REFLECTION-TRAINING-TRANSFER-50` freezes P0037's byte-identical vocabulary on a fresh filename-only all-training `25 ARC1 + 25 ARC2` cohort: `1/50` exact (`1` ARC1, `0` ARC2), with all `49` complete NO reports retained and reproduced byte-for-byte. The sole YES is a pre-existing recolor relation; the P0037 cross-reflection primitive selects zero times. This is limited negative all-training-subset transfer evidence and does not promote that primitive or support an ARC1/ARC2 solver claim.
...
- `P0043-ARC12-SEPARATOR-ARROW-GUIDED-PANEL-STAMP-DEVELOPMENT-40` freezes one new strict corpus-callosum relation against P0042's already exposed identical mixed-partition cohort: `3/40` exact (`+1`; `0/20` ARC1, `3/20` ARC2), with all `37` complete NO reports and 162 artifacts reproduced byte-for-byte. The added ARC2 YES is `separator_arrow_guided_panel_stamp`: it rederives a modal background, a unique full-width divider, three dynamic 3×3 template/arrow/payload roles, lower template-mask agreement, and a fully empty direction-selected stamp region; ambiguity, collision, and out-of-bounds cases refuse. This is same-cohort development only—not fresh transfer, a benchmark score, or an ARC1/ARC2 solver claim—and must be followed by a separately frozen mixed-partition cohort before promotion.
- `P0044-ARC12-SEPARATOR-ARROW-GUIDED-PANEL-STAMP-MIXED-PARTITION-TRANSFER-40` freezes P0043's exact controller bytes on a new globally ID-disjoint `20` ARC1-evaluation plus `20` ARC2-training filename-only cohort before any selected grid is decoded. It is `3/40` exact (`1/20` ARC1, `2/20` ARC2), with all `37` complete NO reports and 162 artifacts reproduced byte-for-byte. The three YES outcomes use pre-existing separated-panel and dihedral macro-tile relations; `separator_arrow_guided_panel_stamp` selects `0` times. This is negative fresh-transfer evidence for the new relation within the explicitly mixed-partition internal protocol—not a benchmark score or ARC1/ARC2 solver claim—and it does not promote the relation.
...
python3 scripts/run_arc12_filename_holdout.py --verify \
  ...
  --arc2-source /path/to/arc2-source
python3 scripts/run_arc12_filename_holdout.py --verify \
  --packet research/packets/P0038_ARC12_CROSS_SEPARATOR_REFLECTION_TRAINING_TRANSFER_50.json \
  --report-root reports/P0038_arc12_cross_separator_reflection_training_transfer_50 \
  --arc1-source /path/to/arc1-source \
  --arc2-source /path/to/arc2-source
python3 scripts/run_arc12_filename_holdout.py --verify \
  --packet research/packets/P0039_ARC12_TRAINING_DEVELOPMENT_BASELINE_40.json \
  --report-root reports/P0039_arc12_training_development_baseline_40 \
  --arc1-source /path/to/arc1-source \
  --arc2-source /path/to/arc2-source
python3 scripts/run_arc12_filename_holdout.py --verify \
  --packet research/packets/P0040_ARC12_UNIFORM_FRAME_SIZE_FILL_DEVELOPMENT_40.json \
  --report-root reports/P0040_arc12_uniform_frame_size_fill_development_40 \
  --arc1-source /path/to/arc1-source \
  --arc2-source /path/to/arc2-source
python3 scripts/run_arc12_filename_holdout.py --verify \
  --packet research/packets/P0041_ARC12_UNIFORM_FRAME_SIZE_FILL_FINAL_TRAINING_TRANSFER_38.json \
...
python3 scripts/run_arc12_filename_holdout.py --verify \
  ...
  --arc2-source /path/to/arc2-source
python3 scripts/run_arc12_filename_holdout.py --verify \
  --packet research/packets/P0043_ARC12_SEPARATOR_ARROW_GUIDED_PANEL_STAMP_DEVELOPMENT_40.json \
  --report-root reports/P0043_arc12_separator_arrow_guided_panel_stamp_development_40 \
  --arc1-source /path/to/arc1-source \
  --arc2-source /path/to/arc2-source
python3 scripts/run_arc12_filename_holdout.py --verify \
  --packet research/packets/P0044_ARC12_SEPARATOR_ARROW_GUIDED_PANEL_STAMP_MIXED_PARTITION_TRANSFER_40.json \
  --report-root reports/P0044_arc12_separator_arrow_guided_panel_stamp_mixed_partition_transfer_40 \
  --arc1-source /path/to/arc1-source \
  --arc2-source /path/to/arc2-source
...
P0031's contiguous-panel/component controller bytes are frozen at `fd3ac79b5415d0f7b42747c5bff19829802ccde3`; P0033's all-training development-baseline bytes are frozen at `5de7928e61ab625be59abab70ce3570e018cbd2e`; P0034's shared-background panel bytes are frozen at `3e04e03086528da6a9a2107a08ebdb4f219bdae4`; P0035's transfer and P0036's development-baseline bytes are frozen at `e869c4842817624925e6576a1be9bb1f27399977`; P0037's development, P0038's transfer, and P0039's development-baseline bytes are frozen at `5593527a16a57bcc0925ae3692b0888f141452e3`; P0040's frame-size relation and future-only concise renderer bytes are frozen at `c3842b7d28fb2829b79ec815a99b1bc39912a189`; P0041's final-transfer provenance bytes are frozen at `3524151a87db4852d8859502f8795d1209bedc35`, with all live answer-selection bytes equal to P0040; P0043's arrow-guided panel-stamp bytes are frozen at `feff068b8e1e1116427f87f59af4e54b922a1dd1` after the P0042 exposed baseline and before P0043 execution, and P0044 verifies those same bytes without post-P0043 controller drift.
...
- `reports/` — retained ARC12 reports through P0044, including complete development and frozen-holdout V&V evidence.
